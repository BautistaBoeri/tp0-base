...
Para soportar el envío por batches, el protocolo ahora funciona de la siguiente manera:

1. **Header de Lote (3 bytes fijos para envío):**
   - **1 byte (OpCode):** Indica el tipo de mensaje (`3` = Batch de apuestas).
   - **2 bytes (Cantidad `N`):** Un entero sin signo (Big-Endian) equivalente a la cantidad de apuestas que vienen en el lote.

2. **Payload de batch (Variable):**
   Contiene las `N` apuestas indicadas, enviadas una detrás de la otra. Cada apuesta individual se compone de:
   - **2 bytes (Largo de apuesta):** Indica los bytes que ocupará el string de esta apuesta puntual.
   - **`L` bytes (Apuesta):** String UTF-8 con el CSV de la apuesta (`Nombre,Apellido,Documento,FechaNacimiento,Numero`).
